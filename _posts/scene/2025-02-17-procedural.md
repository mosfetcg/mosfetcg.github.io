...
```ruby
# 本文β符号表示因子，当不标记其他符号时表示散射量βs。其中R、M表示对应的两种散射。还包含HR和HM。  
βR(h,λ) = 8PI³(n²-1)² |* exp(-h/HR)
           / (3Nλ^4)  |
# h高度 n空气折射率 N海平面分子密度 HR(scale hieght) = 8KM

# 对于蓝光440 绿光550 红光680 左式为
βR = 33.1e-6 13.5e-6 5.8e-6
```
蓝光的波长更短，该值将更高。因此散射越强烈。  
```ruby
# extinction lost
βER = βAR(absorb) + βSR(scat) = βR
...
# g = 0.76
```
参考我们的体积渲染方程，β项可用于光学深度。  
问题是内散射积分S中的使用的光线是什么？(甚至我们正在计算天空光线)。  
这种计算情况下，可简化S为从太阳唯一接受光。有关这一点的区别和影响，笔者不再深入研究。  
初始强度项随着随着大气距离(另计算一条dx)减弱。  
```ruby
Lo = ∫all_line_dx|TSdx (沿着视线减弱)
# 请注意S的模式。

# S(p) = s(p) ∫ρLi
Li = SunBase * T2(sun_dx_line); #(沿着太阳减弱)
Li *= ρ β(h)
S(p) = ∫all_sphere|Li
```
对于每种散射，需要单独计算贡献。  
```ruby
Lo = ∫all_dx|TSdx
   = SunBase ρ ∫all_dx|T*T2*β(h) dx

SkyLo = Rlo + Mlo # 单独计算两个e即可
```
到这里我们的理论就结束了，直接使用上面这个GLSL也是可以的。  
