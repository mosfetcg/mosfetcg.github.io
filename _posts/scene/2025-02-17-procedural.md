...
例如，大气通常不会被直接建模为物理的，只需要一点程序技巧就可以达成一种大气存在的假象  ，这就是为什么算法很流行。  
...
关于近似方法的共同点：1，是否会影响该类对象原本的职责？例如天空还可用于照明，这种情况下不会造成影响。但对于其他对象呢？  
...
这里有很多你不想深入的云的分布和形状的术语。我们特别说明，接触地球表面的非常低的层状云被赋予了雾或薄雾的俗称，这些名称并不包括在分类中。通常模拟它们的技术也可以有所不同。
<div class="x gr txac">
  <div class="x la flex mg0">
...
## 纹理云
这是用3DFBM来生成一些近乎贴合球面映射的云。使用广泛但有限，特别是映射的感觉非常强(弯曲)。  
有时会用因子适当调整视角结果。  

获取FBM的两个样本总是比单个更好，更有层次感，显著减少聚集或者扁平感。  
硬着色，尝试，低密度混合背景和白色，中间为白色，最高为灰色。  
```cpp
