...
```
`雷利散射(Rayleigh scattering)`是光或其他电磁辐射因比其波长小的多的粒子而散射产生的某种现象。该现象以英国物理学家雷利命名。  
该散射说明，较短波长（蓝色）的散射比较长波长（红色）的散射更强烈。这导致天空各个区域都会出现间接的蓝光和紫光。人眼对这种波长组合的反应就像是蓝光和白光的组合。  

## 大气的形状
有时候对大气的形状似乎有些理解偏差，其边界在圆环的可视范围内是一种欧几里得圆段。  
由于大气高度很低(100KM)，而地球半径很大(6300km)，朝着水平方向看见比单个垂直高度多得多的空间是很正常的。  
到大气边缘的近似距离由以下公式近似：  
```
d = 1.0 / dot(v, n)
```
因此该距离更容易评估较远处的云等对象。  
尽管大气的圆环中在任何地方都可能垂直分布一些云。观察中，远处的云通常很小，接近地平线。距离影响大小。这种差异在现实中很明显。  
