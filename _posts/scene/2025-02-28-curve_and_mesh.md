...
我通常只写图像顺序渲染。它们如何处理大型网格？最坏情况下，每个像素需要遍历所有三角形，这种计算量超出想象。  
最紧迫的一个结论可能是，利用空间结构进行编排的可能性为99.99%。  
第二是，使用射线直接求解交叉比SDF更快。目前最直接、合适的已有选择可能先选择一个路径追踪器作为模板。  

最后就是实际问题，在单个着色器中处理这些数据？唯一的选择是硬编码它们进行编译，包括加速结构的交叉测试，完全手工。  
```
3488
