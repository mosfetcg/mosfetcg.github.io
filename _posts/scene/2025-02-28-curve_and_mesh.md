...
在实践中，插值方案通常具有不良属性，通常优选仅近似点的曲线方案。  
近似方案中，**控制点**会影响曲线的形状，但不能准确指定通过它们。但它们可以控制形状。  

最重要的两种近似曲线是`Bezier(贝塞尔)`和`B-Spline`曲线。  
贝塞尔曲线，是自由曲线的最常见表示形式之一。`d`度(degree)曲线由`d+1`个控制点控制。曲线仅插入第一个和最后一个控制点。形状直接受其他点的影响。  
通常，复杂的形状是通过连接许多低度的曲线而制成的，在计算机图形中，三次立方cubic贝塞尔曲线(d=3)通常用于此目的。我们给出下列表达式。  
```ruby
f(u) =(1-3u+3u^2-u^3)p1 +(3u-6u^2+3u^3)p2 +(3u^2-3u^3)p3 +(u^3)p4
```