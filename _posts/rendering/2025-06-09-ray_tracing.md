...
## 高级直接照明
#### 直接照明
从更高层次考虑设计总有助于理解你的渲染的正确性，目前求解的积分可以做什么？以及对应的目的在物理上正确的吗？  

例如，您可能还记得有一个求解直接光照的积分形式，它仅对灯源区域进行采样，甚至仅求解此特定形状的积分。(阴影光线参与计算给定点的直接光照)。  

仅上述一例就可以说明问题(10-8)。  
现在想象半球方式，即使我们正确计算了带有阴影遮挡的光线，仍然可能大量对无效区域进行采样，由于样本均匀，黑色样本也被加入到积分中，核心点是，
尽管**积分的表达式不变**，但该项的结果却不是物理上正确的，因为现实中，该偏转了光源的光线不会落入虚空，而是由光源更多发送光线到此方向上，即使它们没有在方向上直接相交。  

一旦理解此问题，重要性采样就有意义。当然，直接光照的积分本身就算一种效率极高的采样(对正确结果)。我们将半球采样的样本重新按光源分布，从而减少错误样本的配重(消除噪声)，这得到相同的结果，并且半球的积分仍然在采样的域上不变。因此，需要找出对应的`/pdvi`。  

这可能是同一事物的两种实现方式，第一种直接改变散射(因为确实偏转光线应该被导向光源，因为向其发送了更多光线)，第二种仍然采样无效区域，但完全屏蔽错误的结果。  

#### 环境纹理光源(Environment Map Lights)
用户很容易想要将环境光源(纹理图)设置为光源，最好根据其亮度分布进行重要性采样。  
另外，一起使用环境和单独光源时，也需要注意实现上的区别，但这里并不是关注此问题。  

#### 重要性采样+(MIS, Multiple Importance Sampling)
有几种不同的IS方式，针对某种光源。  
第一是均匀采样，如前所述，对单一光源将引入噪声，或者低亮度结果。物理上通常不正确的，除非亮度相近且充满世界。  
第二是光源采样，如前所述。  
第三称为BRDF IS，这是自然的，因为采样那些有效视觉区域更好。  

将多种样本的结果平均即得MIS。  
我们也可以互相思考这些问题，只在光源采样可能不匹配BRDF，只在BRDF采样可能不会命中光源。  
新MIS估值器的数学表示如下(27)：  
```ruby
