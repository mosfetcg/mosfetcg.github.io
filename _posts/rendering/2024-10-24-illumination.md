...
## 近似全局照明
对大型景观进行户外照明时，间接照明的贡献只是**适度**且可预测的。因此近似方法是优先选择。  
让场景由3到4个定向灯、每个光源配置一些阴影、一些环境光遮蔽，最后一个雾层组成。  

太阳光是主光。它呈黄白色且非常强烈，强度大约1.5。其阴影稍微柔和一些，可以将半影着色并过度饱和为某种红色或橙色。  
太阳是如此小的光源，不要用AO来调节太阳光。阴影已经处理遮挡问题。  

定向天空光为次要白蓝色光源，可以随角度选择颜色。亮度0.2就足够了。乘以AO留给这个补光灯而不是全部方程。  

最后一个光源用于模拟间接照明，有限，复制一份反方向太阳光模拟反射，强度为1.25。可以乘以场景颜色。  
