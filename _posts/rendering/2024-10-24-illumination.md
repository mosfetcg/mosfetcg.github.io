...
## 近似全局照明
对大型景观进行户外照明时，间接照明的贡献只是**适度**且可预测的。因此代替方案是优先选择。  
场景可以由3至4个定向灯、一些阴影、一些环境光遮蔽和一个雾层组成。  

太阳光是主光。它呈黄白色且非常强烈，强度大约1.5。其阴影稍微柔和一些，可以将半影着色并过度饱和为某种红色或橙色。  
太阳是如此小的光源，不要用环境遮挡来调节太阳光。阴影已经直接处理了这个问题。  

垂直天空光为次要白蓝色光源。亮度0.2就足够了。AO正好应该留给这个补光灯。然后乘以AO。  
最后一个光源用于模拟间接照明，有限，复制一份反方向太阳光模拟反射，强度为1.25。  
