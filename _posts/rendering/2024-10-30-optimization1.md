...
第二个概念是x~p的期望，这里原文也有点模糊。另外，`x`通常作为函数解释，而不是格的解释，但后两项可以作为格解释。  
...
有兴趣的读者可以研究一下和比率方法有无联系，1可以替换为概率的积分(可能已经很明显)，而我们也对所有可能进行了采样。  

#### 估值
原文这里有一些未知联系，尽管这些材料可能是正确的表述。目前应该将IST作为引用。  
```ruby
         I(f) = ∫S|f(du)du
         E(f) = ∫S|f(du)p(du)du          # 证明如前所述
       f(dui) = sample_result, dui~p
    EST(fdui) = 1/N * Σ(i=1..N)f(dui).   # 事实，所有按xi~p产生的样本的均值近似于期望
  #estimator
...
```
首先，估计中iid样本的必要条件是PDF`xi~p`，两式相等正确的原因在于假设xi样本已遵循此分布，否则，就不是同一个期望值。我们已经在上面证明了这一点。  
